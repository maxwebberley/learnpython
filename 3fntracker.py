...
def whatnow1():
    whatnow=input("Press enter to continue. Type bg to see your best game. Type wg to see your worst game. > ")
    if whatnow=="":
        addgame()
    else:
        if amountofgames==0:
...
def kill():
    ...
    cls()
    print ("-=-=-=-= NEW GAME=-=-=-=- \n \n")
    a=eval(input("Number of kills > (type 'stats' to see stats) "))
    if a=="stats":
        stats()
    elif type(a) is int:
        kills.append(a)
